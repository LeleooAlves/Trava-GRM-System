import { ExpertAPI } from './api_expert.js';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const expertApi = new ExpertAPI('9rT29GUPUUFBJubZLg1XTMHZAWmMN9yq', 7512);
    ...
    let lastAnalyzedKeyword = "";

    async function analyzeKeywordWithAI(keyword) {
        const aiSection = document.getElementById('ai-specialist-section');
        ...
        try {
            const prompt = `Atue como um Especialista de Produtos. Analise a seguinte keyword pesquisada pelo usuário e extraia/interprete buscando ao máximo a intenção:
 Keyword: "${keyword}"
 
 RESPONDA EXATAMENTE NO FORMATO ABAIXO:
 
 Kw: [Palavra-chave informada]
 Typo: [Se houver erro óbvio de digitação pelo contexto, acento ausente, letra errada, ou espaço onde não deveria ter, corrija aqui como o Google faria ("Você quis dizer: ..."). Ex: "ppte de concreto para cachorro" -> "pote de concreto para cachorro"; "dental clean" -> "dentalclean"; "deconcreto" -> "de concreto". Se não houver erro, escreva: Vazio]
 Descrição: [Breve resumo/AI Overview explicando o que é o produto, se existe e onde geralmente é achado. Se for invenção, sugira o que o usuário provavelmente quis dizer.]
 Marca: [ATENÇÃO: Extraia APENAS se houver a presença de uma marca DETECTADA DENTRO da própria keyword. Pode estar escrita errada ou separada (Ex: "escova dental clean" -> Dentalclean, "tenis niki" -> Nike). NÃO sugira marcas relacionadas. Se não constar o nome de NENHUMA marca na keyword lida, responda estritamente: "Nenhuma marca identificada"]
 Língua: [Português ou Inglês. Alertar se não for, lembrando que nomes de marcas contam como exceções]`;

            const response = await expertApi.chatCompletion(
            ...
            const formatted = rawResponse
                .replace(/(Kw:|Typo:|Descrição:|Marca:|Língua:)/gi, '<b>$1</b>')
                .replace(/\n/g, '<br>');
            
            aiContent.innerHTML = `<div style="padding-top:4px;">${formatted}</div>`;
        } catch (error) {
            console.error("AI Analysis Error:", error);
            aiContent.innerHTML = `<span style="color:var(--red);">Erro ao analisar keyword.</span>`;
        }
    }

    // Function to repeatedly check keyword since Shopee uses React (SPA)
    async function fetchKeywordContinuously() {
        try {
            const tab = await getActiveTab();
            if (tab && tab.url && tab.url.includes('shopee.io')) {
                chrome.tabs.sendMessage(tab.id, { action: "get_keyword" }, (response) => {
                    if (chrome.runtime.lastError) {
                        // Keep previous value if there's a temporary connection drop
                        return;
                    }
                    if (response && response.keyword) {
                        kwValueEl.innerText = response.keyword;
                        
                        // NEW: Dispara a IA caso a keyword seja diferente da anterior
                        const currentKw = response.keyword.trim();
                        if (currentKw.length > 0 && currentKw !== lastAnalyzedKeyword && currentKw !== "Unknown") {
                            lastAnalyzedKeyword = currentKw;
                            analyzeKeywordWithAI(currentKw);
                        }
                    } else {
                        if (kwValueEl.innerText === "") kwValueEl.innerText = "Unknown";
                    }
                });
            } else {
                if (!tab) return;
    ...
    // Call immediately and also poll every 1 second
    fetchKeywordContinuously();
    setInterval(fetchKeywordContinuously, 1000);
