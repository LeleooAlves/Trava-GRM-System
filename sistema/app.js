...
function setupRealtime() {
    ...
    function sendSeatalkWebhook(projectName) {
        // Use Vite proxy (/webhook/seatalk) in dev to avoid CORS.
        // In production, configure a server-side proxy or Edge Function to forward this path.
        const webhookUrl = '/webhook/seatalk';
        fetch(webhookUrl, {
            method: 'POST',
            ...
            body: JSON.stringify({
                tag: "text",
                text: {
                    content: `Projeto Finalizado: ${projectName}`
                }
            })
        }).catch(err => console.error('[Webhook] Error:', err));
    }

    async function checkAndNotifyIfAllGoalsMet(projectId) {
        if (!projectId || notifiedProjects.has(projectId)) return;

        // OPTIMISTIC LOCK: add immediately to prevent parallel async calls from firing multiple times
        notifiedProjects.add(projectId);
        ...
        if (error || !goals || goals.length === 0) {
            notifiedProjects.delete(projectId); // rollback if bad data
            return;
        }

        // Only consider types that have a target set
        ...
        const allMet = activeGoals.every(g => Number(g.current_amount) >= Number(g.target_amount));
        if (!allMet) {
            notifiedProjects.delete(projectId); // not done yet, allow future re-check
            return;
        }
        ...
        // Only fire if not already manually finalized
        if (project && project.status !== 'completed') {
            console.log(`[Webhook] Todas as metas atingidas! Notificando para: ${project.name}`);
            sendSeatalkWebhook(project.name);
        }
    }
    ...
    const goalsChannel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'project_goals' }, payload => {
            ...
            if (currentProjectId && payload.new && payload.new.project_id === currentProjectId) {
                ...
                // 3. Check if ALL goals are now met and notify via webhook
                checkAndNotifyIfAllGoalsMet(currentProjectId);
            }
        })
        .subscribe((status) => {
...
function updateUI() {
    ...
    // Use sorted types for consistent layout
    const types = ['0-20', '20-50', '50-80', '80-100'];
...
async function loadCompletedProjects(searchTerm = '') {
    ...
    projects.forEach(project => {
        ...
        // Defined types in order
        const types = ['0-20', '20-50', '50-80', '80-100'];
        types.forEach(type => {
            const goal = projectGoals.find(g => g.type_key === type) || { current_amount: 0 };
...
function generateAndCopyReport() {
    try {
        ...
        // Canvas Size
        canvas.width = MARGIN * 2 + (CARD_W * 4) + (GAP * 3);
        ...
        types.forEach((type, index) => {
            ...
            ctx.fillText(type, x + CARD_W / 2, y + 22.5);
